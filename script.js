...
function setupEventListeners() {btnAddLigante.addEventListener('click', () => { if ((currentLigantes + currentLivres) < 6 && currentLigantes < 6) { currentLigantes++; updateSimulator(); } });
                                ...
                                const start = (e) => { drag = true; const c = e.touches ? e.touches[0] : e; prev = { x: c.clientX, y: c.clientY }; };const move = (e) => {if (!drag) return; const c = e.touches ? e.touches[0] : e;moleculeGroup.rotation.y += (c.clientX - prev.x) * 0.007; moleculeGroup.rotation.x += (c.clientY - prev.y) * 0.007;prev = { x: c.clientX, y: c.clientY };};container3d.addEventListener('mousedown', start); container3d.addEventListener('mousemove', move);window.addEventListener('mouseup', () => drag = false);container3d.addEventListener('touchstart', start, { passive: true }); container3d.addEventListener('touchmove', move, { passive: true });window.addEventListener('touchend', () => drag = false);window.addEventListener('resize', () => {camera.aspect = container3d.clientWidth / container3d.clientHeight; camera.updateProjectionMatrix(); renderer.setSize(container3d.clientWidth, container3d.clientHeight);});}function animate() { requestAnimationFrame(animate); if(moleculeGroup && !drag) moleculeGroup.rotation.y += 0.003; if(renderer) renderer.render(scene, camera); }let drag = false;window.addEventListener('mousedown', () => drag = true); window.addEventListener('mouseup', () => drag = false);window.addEventListener('touchstart', () => drag = true); window.addEventListener('touchend', () => drag = false);init();
